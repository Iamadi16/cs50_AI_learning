...
class Space():
    ...
    def hill_climb(self, maximum = None, image_prefix = None, log = False):
        ...
        if log:
            print("first: cost", self.get_cost(self.hospitals))
        if image_prefix:
    ...
    def ran_restart(self, maximum, image_prefix = None, log = False):
        best_hospitals = None
        best_cost = None

        for i in range(maximum):
            self.hospitals = self.hill_climb()
            cost = self.get_cost(self.hospitals)
            if best_cost is None or cost < best_cost:
                best_cost = cost
                best_hospitals = self.hospitals
                if log:
...
hospitals = s.ran_restart(maximum=10, image_prefix="hospitals", log=True)